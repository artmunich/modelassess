!This program is written to calculate the error evlovement of CFSR compared with true value.
...
data year/'1999','2000','2001','2002','2003','2004','2005','2006','2007','2008','2009','2010'/
	...
	open(200,file='H:\cfsrhgt500\cfsr_hgt.grd',form='unformatted',access='direct',recl=nx1*ny1) !!True values
	open(10,file='H:\variance_fortran\errorfield_abs.grd',form='unformatted',access='direct',recl=nx_new*ny_new*inter) !!Store absolute difference values.
...
do k=1,12
	...
	!pause
	fname=a//year(k)//b
	open(100,file='H:\ncep_cfs_h500_rawdata\'//trim(fname)//'',form='unformatted',access='direct',recl=nx1*ny1) !Raw data

	do j=1,np !!1460 Forecasts viewed as points.
	flag=flag+1
	ini1=(j-1)*inter+1
	ini2=iob
	write(*,*)ini1,ini2
...
!Output:
!   hnew: Absoulute difference value.
!--------------------------------------------------------------------------------
